...
public class VariablesObjectPattern extends AbstractPattern implements IPattern {
	...
	public PictogramElement add(IAddContext context) {
		...
		// Add text
		{
			String constantString = constant.getQualifier() + " " + constant.getName();
			if(constant.isIsArray())
				constantString += "[]";
			constantString += ": ";
			constantString += constant.getType();
			constantString += " = ";
			constantString += constant.getValue();
			
			Text text = createService.createText(constantRoundedRectangle, constantString);
	...
	public Object[] create(ICreateContext context) {
        ...
        HashMap<String, String> info = Prompts.askConstantInfo();
        constant.setName(info.get(IDEConstants.NAME));
        constant.setType(info.get(IDEConstants.TYPE));
        constant.setValue(info.get(IDEConstants.VALUE));
        constant.setQualifier(info.get(IDEConstants.QUALIFIER));
        if(info.get(IDEConstants.IS_ARRAY).equalsIgnoreCase(IDEConstants.TRUE))
        	constant.setIsArray(true);
        else
        	constant.setIsArray(false);
